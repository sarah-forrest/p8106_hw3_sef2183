...
# MARS model
roc_mars <- roc(auto$mpg_cat[-rowTrain], mars.pred)
plot(roc_mars, legacy.axes = TRUE, print.auc = TRUE)
plot(smooth(roc_mars), col = 4, add = TRUE)
```

The AUC for the GLM model is 0.963, indicating that the prediction performance is very good. The misclassification error rate is [].

The AUC for the MARS model is slightly lower, at 0.943. Although lower than the GLM model, this AUC indicates that the prediction performance is very good for the MARS model as well. The misclassification error rate is [].